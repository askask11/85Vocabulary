...
public class Randomizer
{
    ...
    public static int[] randomInts(int lowerBound, int upperBound, int length)
    {
        int[] seq = new int[length];
        for (int i = 0; i < length; i++)
        {
            seq[i] = randomInt(lowerBound, upperBound);
        }
        return seq;
    }
     ...
     * @param letters A set of chars that can appear in the random sequence
     * generated.
    ...
    public static int[] trueRandomSet(int lowerBound, int upperBound) throws InterruptedException, IOException
    {
        String c = Commander.executeCommand("curl https://www.random.org/sequences/?min=" + lowerBound + "&max=" + upperBound + "&col=" + upperBound + "&format=plain&rnd=new");
        String[] cs = c.split("	");
        int[] set = new int[cs.length];
        for (int i = 0; i < cs.length; i++)
        {
            String js = cs[i];
            set[i] = Integer.parseInt(js);
        }
        return set;
    }

    public static int[] trueRandomSet(int lowerBound, int upperBound, int length) throws InterruptedException, IOException
    {
        int[] set = new int[length];
        for (int i = 0; i < length; i++)
        {
            int[] set0 = trueRandomSet(lowerBound, upperBound);
            set[i] = set0[i];
        }
        return set;
    }
